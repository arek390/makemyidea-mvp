...
const translations: Record<Language, Translations> = {
  ...
  spaceListHint: 'Wählen Sie bis zu 5 aus.',
    ...
    questionTemplates: (spaceDef, timeDef) => [
      `Wie könnte "${spaceDef}" auf "${timeDef}" reagieren und eine neue Chance eröffnen?`,
      `Wie erlebt der Nutzer "${timeDef}" innerhalb von "${spaceDef}"?`,
      `Welche Einschränkungen von "${spaceDef}" sind während "${timeDef}" am wichtigsten?`,
      `Was könnte in "${spaceDef}" vereinfacht werden, um bei "${timeDef}" zu bestehen?`,
      `Welche Risiken oder Chancen entstehen für "${spaceDef}" in "${timeDef}"?`,
      `Welche State-of-the-Art-Technik verbessert "${spaceDef}" für "${timeDef}"?`,
      `Wo liegen die größten Ineffizienzen von "${spaceDef}" während "${timeDef}"?`,
      `Welche Daten oder Signale fehlen, um "${spaceDef}" in "${timeDef}" zu optimieren?`,
      `Wie verändern Nachhaltigkeit oder Compliance "${spaceDef}" in "${timeDef}"?`,
      `Welcher neue Kundennutzen entsteht in "${spaceDef}" für "${timeDef}"?`,
    ],
    llmIdeaTemplate: (spaceDef, timeDef) =>
...
const IconQuestion = () => (
  <svg viewBox="0 0 24 24" aria-hidden="true">
    ...
    />
    <path
      d="M9.5 9.2a2.7 2.7 0 1 1 4.1 2.3c-.8.5-1.2 1-1.2 2.1"
      fill="none"
      stroke="currentColor"
      strokeWidth="1.5"
      strokeLinecap="round"
...
function App() {
  ...
  const [spaceOptions, setSpaceOptions] = useState<OptionItem[]>(() => {
    const world = buildWorldSuggestions('', translations.English).slice(0, 10)
    const elements = buildElementSuggestions('', translations.English, 'English').slice(0, 10)
    return [
  ...
  const [timeOptions, setTimeOptions] = useState<TimeOptionItem[]>(
    translations.English.timeSuggestions.map((label, index) => ({ id: index, label }))
  )
  ...
  const [reportLanguage, setReportLanguage] = useState<Language>('English')
